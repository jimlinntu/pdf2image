...
for image_p in current.iterdir():
    if image_p.is_file() and re.search("\.(jpe?g|png)$", image_p.name) is not None:
        number_only_name = re.sub("[^0-9]", "", image_p.name)
        number = re.search("[0-9]+", number_only_name)
...
start_image = exif_rotate(Image.open(start_image_p).convert("RGB"))
append_images = []
